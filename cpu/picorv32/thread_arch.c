 ...
 *
 * @file        cpu.c
 * @brief       Implementation of the CPU thread management for picorv32
 *
 * @author      Tomas Styblo
 * @}
 */
...
char *thread_stack_init(thread_task_func_t task_func,
                             ...
                             int stack_size)
{
    DEBUG("thread_arch.c - thread_stack_init - START: stack_start=%p, stack_size=%d - DEBUG\n", stack_start, stack_size);
    ...
    *stk_top = STACK_MARKER;
    ...
    DEBUG("thread_arch.c - thread_stack_init - END: stk_top=%p\n", stk_top);
    return (char *) stk_top;
}
...
{
    DEBUG_PUTS("thread_arch.c - cpu_switch_context_exit - START");
    /* enable interrupts */
    irq_enable();
