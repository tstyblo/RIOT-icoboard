...
{
    ICODEBUG("nanostubs.c - nanostubs_init - START")
#if defined(MODULE_STDIO_UART)
    /* STDIO redirected to UART, no line buffering */
    ICODEBUG("nanostubs.c - nanostubs_init - stdio_init")
    stdio_init();
    ICODEBUG("nanostubs.c - nanostubs_init - setvbuf")
    // setvbuf(stdout, NULL, _IONBF, 0);
#endif
    ICODEBUG("nanostubs.c - nanostubs_init - END")
}
...
{
    ICODEBUG("nanostubs.c - _sbrk - START")
    ...
    heap_end += incr;
    ICODEBUG("nanostubs.c - _sbrk - END")
...
{
    //ICODEBUG("nanostubs.c - _read - START")
#if defined(MODULE_STDIO_UART)
    if (fd == STDIN_FILENO) {
        //ICODEBUG("nanostubs.c - _read - ok")
        return stdio_read(buffer, count);
    }
    //ICODEBUG("nanostubs.c - _read - error")
    errno = EBADF;
    return -1;
#else
    (void) fd;
...
{
    //ICODEBUG("nanostubs.c - _write - START")
#if defined(MODULE_STDIO_UART)
    //ICODEBUG("nanostubs.c - _write - uart")
    if (fd == STDOUT_FILENO || fd == STDERR_FILENO) {
        //ICODEBUG("nanostubs.c - _write - ok")
        return stdio_write(data, count);
    }
    errno = EBADF;
    //ICODEBUG("nanostubs.c - _write - error")
    return -1;
#else
    (void) fd; 
    (void) data;
